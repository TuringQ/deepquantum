...
class UnitaryDecomposer():
    ...
    def decomp(self, method: str) -> dict:
        """
        method 前三个字母的含义如下
        <第0个字母>
        r: Reck架构
        ...
        if method not in ["rssr","rsdr","rdsr","rddr","rssl","rsdl","rdsl","rddl",\
            ...
            if method[-1] == 'r':
                return decomp_cr(self.U, method)
            elif method[-1] == 'l':
                return decomp_cl(self.U, method)
        elif method[0] == 'r':
            if method[-1] == 'r':
                return decomp_rr(self.U, method)
            elif method[-1] == 'l':
                return decomp_rl(self.U, method)

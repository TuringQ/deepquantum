...
from .photonic_qmath import fock_combinations, permanent, product_factorial, sort_dict_fock_basis, sub_matrix
...
class QumodeCircuit(Operation):
    ...
    def _forward_helper_basis(self, data = None, state = None, is_prob = False):
        ...
        for i in range(len(final_states)):
            final_state = FockState(nmode=self.nmode, cutoff=self.cutoff, state=final_states[i], basis=self.basis)
            # final_state = tuple(final_states[i].tolist())
            # out_dict[final_states[i]] = rst[i]
    ...
    def evolve(self, init_state = None):
        ...
        """
        out_dic = {}
        output_list = self.get_all_fock_basis(init_state)
        for ii in output_list:
            f_state = ii
            if max(f_state) < self.cutoff:
                out_dic[ii] = self.get_amplitude(f_state, init_state).reshape(1)
        return sort_dict_fock_basis(out_dic)

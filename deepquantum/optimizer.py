...
class OptimizerSPSA(Optimizer):
    def __init__(self, target_func, param_init, random_state=0, SPSA_hyperparam=SPSA_default_hyperparam):
        '''初始化内容包括：
        ...
        self.hyperparam = SPSA_hyperparam
        self.iter = 0
...
class OptimizerFourier(Optimizer):
    def __init__(self, target_func, param_init, random_state=0, R=5, lr=0.1):
        '''初始化内容包括：
        1）傅里叶级数的个数R
    ...
    def param_register(self,param_array:np.ndarray,target:np.ndarray):
        ...
        for param_id in range(self.nparam):
            self.u[param_id*(2*self.R+1):(1+param_id)*(2*self.R+1)] = np.linalg.solve(self.A,target[param_id*(2*self.R+1):(1+param_id)*(2*self.R+1)])
        ...
        for param_id in range(self.nparam):
            ...
            grad[param_id] = -(np.arange(1,self.R+1)*np.sin(theta*np.arange(1,self.R+1)))@\
                            self.u[1+param_id*(2*self.R+1):self.R+1+param_id*(2*self.R+1)]+(np.arange(1,self.R+1)*\
                            np.cos(theta*np.arange(1,self.R+1)))@self.u[self.R+1+param_id*(2*self.R+1):self.R*2+1+param_id*(2*self.R+1)]

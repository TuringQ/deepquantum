...
# 优化器
opt_d = optim.SGD(discriminator.parameters(), lr=lr_d)
opt_g = optim.SGD(generator.parameters(), lr=lr_g)
...
while True:
    for _, (data, _) in enumerate(dataloader):
        ...
        out_d_real = discriminator(real_data).view(-1)
        out_d_fake = discriminator(fake_data.detach()).view(-1)

        err_d_real = criterion(out_d_real, real_labels)
        err_d_fake = criterion(out_d_fake, fake_labels)
        # 传播梯度
        err_d_real.backward()
        err_d_fake.backward()
        ...
        # 训练生成器
        generator.zero_grad()
        out_d_fake = discriminator(fake_data).view(-1)
        err_g = criterion(out_d_fake, real_labels)
        err_g.backward()
        opt_g.step()
        ...
        # 显示损失值
        if counter % 10 == 0:
            print(f'迭代次数: {counter}, 鉴别器损失: {err_d:0.3f}, 生成器损失: {err_g:0.3f}')

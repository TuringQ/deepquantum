...
def permanent_dq(nmode, batch_size):
    ...
    def get_perm_dq(matrix):
        ...
        for i in range(trials):
            if nmode > 21 and batch_size >= 100:
                torch.vmap(permanent, chunk_size=1)(matrix[i * batch_size : (i + 1) * batch_size])
            if nmode == 18 and batch_size == 1000:
                torch.vmap(permanent, chunk_size=200)(matrix[i * batch_size : (i + 1) * batch_size])
            else:

...
def torontonian_dq(nmode, batch_size):
    ...
    def get_torontonian_dq(matrix):
        ...
        if batch_size == 100 or batch_size == 1000:
            trials = 1
        gamma = torch.diagonal(matrix[0:1], dim1=1, dim2=2)
        torch.vmap(torontonian)(matrix[0:1], gamma)
        time0 = time.time()
        for i in range(trials):
            gamma = torch.diagonal(matrix[i * batch_size : (i + 1) * batch_size], dim1=1, dim2=2)
            torch.vmap(torontonian)(matrix[i * batch_size : (i + 1) * batch_size], gamma)
        time1 = time.time()

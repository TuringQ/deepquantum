...
# 该算法包括以下步骤：
#
# 1. 两个 $n$ 比特输入寄存器初始化为零态：   $$   \lvert \psi_1 \rangle = \lvert 0 \rangle^{\otimes n} \lvert 0 \rangle^{\otimes n}     $$
#
# 2. 对第一个寄存器应用 Hadamard 变换：   $$   \lvert \psi_2 \rangle = \frac{1}{\sqrt{2^n}} \sum_{x \in \{0,1\}^{n} } \lvert x \rangle\lvert 0 \rangle^{\otimes n}   $$
#
# 3. 应用查询函数 $\text{Q}_f$：   $$   \lvert \psi_3 \rangle = \frac{1}{\sqrt{2^n}} \sum_{x \in \{0,1\}^{n} } \lvert x \rangle \lvert f(x) \rangle     $$
#
# 4. 测量第二个寄存器。将观察到 $f(x)$ 的某个值。由于问题的设置，观察到的值 $f(x)$ 可能对应两个可能的输入：$x$ 和 $y = x \oplus b$。因此第一个寄存器变为：     $$   \lvert \psi_4 \rangle = \frac{1}{\sqrt{2}}  \left( \lvert x \rangle + \lvert y \rangle \right)   $$   这里省略了第二个寄存器，因为它已被测量。
#
# 5. 对第一个寄存器应用 Hadamard：   $$   \lvert \psi_5 \rangle = \frac{1}{\sqrt{2^{n+1}}} \sum_{z \in \{0,1\}^{n} } \left[  (-1)^{x \cdot z} + (-1)^{y \cdot z} \right]  \lvert z \rangle   $$
#
# 6. 只有在以下条件下，测量第一个寄存器才会给出输出：   $$   (-1)^{x \cdot z} = (-1)^{y \cdot z}     $$   这意味着:   $$   x \cdot z = y \cdot z \\   x \cdot z = \left( x \oplus b \right) \cdot z \\     x \cdot z = x \cdot z \oplus b \cdot z \\   b \cdot z = 0 \text{ (mod 2)}   $$   将测量一个字符串 $z$，其与 $b$ 的内积为0。因此，重复算法 $\approx n$ 次，我们将能够获得 $n$ 个不同的 $z$ 值，并可以写出以下方程组：   $$   \begin{cases} b \cdot z_1 = 0 \\ b \cdot z_2 = 0 \\ \quad \vdots \\ b \cdot z_n = 0 \end{cases}     $$   先按位计算 $b$ 和 $z_n$ 的乘积，然后再将所有的乘积结果求和，最后看这个和是偶数还是奇数。由此可以确定 $b$，例如通过高斯消元法。
#
...
# %% [markdown]
# 1. 两个 $2$ 量子比特输入寄存器被初始化为零态：   $$   \lvert \psi_1 \rangle = \lvert 0 0 \rangle_1 \lvert 0 0 \rangle_2   $$
#
# 2. 对第一个寄存器中的量子比特应用 Hadamard 门：   $$   \lvert \psi_2 \rangle = \frac{1}{2} \left( \lvert 0 0 \rangle_1 + \lvert 0 1 \rangle_1 + \lvert 1 0 \rangle_1 + \lvert 1 1 \rangle_1 \right) \lvert 0 0 \rangle_2   $$
#
# 3. 对于字符串 $b=11$，查询函数可以实现为 $\text{Q}_f = CX_{1_a 2_a}CX_{1_a 2_b}CX_{1_b 2_a}CX_{1_b 2_b}$（如上图线路图所示）：   $$   \begin{aligned}   \lvert \psi_3 \rangle  = \frac{1}{2} ( \;    & \lvert 0 0 \rangle_1 \; \lvert 0\oplus 0 \oplus 0, & 0 \oplus 0 \oplus 0 \rangle_2 &\\[5pt]   + & \lvert 0 1 \rangle_1 \; \lvert 0\oplus 0 \oplus 1, & 0 \oplus 0 \oplus 1 \rangle_2 &\\[6pt]   + & \lvert 1 0 \rangle_1 \; \lvert 0\oplus 1 \oplus 0, & 0 \oplus 1 \oplus 0 \rangle_2 &\\[6pt]   + & \lvert 1 1 \rangle_1 \; \lvert 0\oplus 1 \oplus 1, & 0 \oplus 1 \oplus 1 \rangle_2 & \; )\\   \end{aligned}   $$   因此：   $$   \begin{aligned}    \lvert \psi_3 \rangle = \frac{1}{2} ( \quad   & \lvert 0 0 \rangle_1  \lvert 0 0 \rangle_2 & \\[6pt]   + & \lvert 0 1 \rangle_1 \lvert 1  1 \rangle_2 & \\[6pt]   + & \lvert 1 0 \rangle_1 \lvert  1   1  \rangle_2 & \\[6pt]   + & \lvert 1 1 \rangle_1 \lvert 0 0 \rangle_2 & \; )\\   \end{aligned}   $$    
#
# 4. 我们测量第二个寄存器。有 $50\%$ 的概率会看到 $\lvert  0   0  \rangle_2$ 或 $\lvert  1   1  \rangle_2$。为了示例的目的，让我们假设看到了 $\lvert  1   1  \rangle_2$。此时系统的状态为   $$   \lvert \psi_4 \rangle = \frac{1}{\sqrt{2}}  \left( \lvert  0   1  \rangle_1 + \lvert  1   0  \rangle_1 \right)   $$   其中我们省略了第二个寄存器，因为它已经被测量过了。
#     

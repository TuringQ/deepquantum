...
dic = io.loadmat('./data/boson_coeff2.mat')
g1 = dic['g1'][0]
g2 = dic['g2'][0]
g3 = dic['g3'][0]
g4 = dic['g4'][0]
...
import torch
# 创建一个张量u1，形状为(3, 2)
u1 = torch.tensor([[[ 0.9665+0.0035j, -0.1915+0.1710j]],
            [[ 0.1830+0.1801j,  0.9655+0.0433j]]])

# 创建一个张量input_state，形状也为(3, 2)，指定每个元素重复的次数
input_state = torch.tensor([0, 1])
print(u1.shape)
...
openfermion_h2 = np.load('./data/openfermion_h2_v3.npy')
openfermion_h2_fci = np.load('./data/openfermion_h2_fci.npy')
# # %matplotlib notebook
fig =  plt.figure()

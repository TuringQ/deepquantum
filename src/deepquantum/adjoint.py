    ...
    def backward(ctx, grad_out: torch.Tensor) -> Tuple[None, ...]:
        ...
        for op in ctx.operators[::-1]:
            ...
            for gate in gates[::-1]:
                ...
                if gate.npara > 0 and parameters[-idx].requires_grad:
                    du_dx = gate.get_derivative(parameters[-idx]).flatten(0, -3) # (npara, 2**n, 2**n)
                    ...
                    for mat in du_dx:
                        ...
                        if isinstance(gate, SingleGate):
                            if len(gate.controls) == 0:
                                ...
                                state_mu = dist_many_ctrl_one_targ_gate(state_mu, targets[:-1], targets[-1], mat, True)
                        else:

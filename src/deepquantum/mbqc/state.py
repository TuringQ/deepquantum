...
class SubGraphState(nn.Module):
    ...
    def set_graph(
        ...
        # assert nx.is_connected(graph), 'Graph is not connected, please use multiple SubGraphState'
        self.graph = graph
        self.nodes_state = nodes_state
        self.update_nodes2wires_dict()
    ...
    def compose(self, other: 'SubGraphState', relabel: bool = True) -> 'SubGraphState':
        ...
        return SubGraphState(nodes_state, state, graph.edges(data=True), graph.nodes)

    def update_nodes2wires_dict(self) -> Dict:
        wires = inverse_permutation(np.argsort(self.nodes).tolist())
        self.nodes2wires_dict = {node: wire for node, wire in zip(self.nodes, wires)}
        return self.nodes2wires_dict

...
class UnitaryMapper:
    ...
    def __init__(
        self, nqubit: int, nmode: int, ugate: Any, success: float, aux: list | None = None, aux_pos: list | None = None
    ) -> None:
        ...
        for t in range(total_trials):
            ...
            for i in range(trials):
                ...
                if re1.success and sum(abs(temp_eqs)) < precision:
                    ...
                    re3 = UnitaryMapper.exchange(
                        re2, self.aux_position
                    )  # need row and column exchange for target aux_pos
                    result.append(re3)
